...
async function fixAllSkinsCompleteWithHistory() {
  try {
    console.log('🚀 Starting COMPLETE skin data fix with MARKET DATA + HISTORY...');
    
    // Get total count of skins
    const totalSkins = await prisma.skin.count();
    console.log(`📊 Total skins in database: ${totalSkins}`);
    
    // Process skins in batches of 25 (smaller batches for better performance with history)
    const batchSize = 25;
    ...
    console.log(`\n🎉 COMPLETE SKIN DATA FIX WITH HISTORY FINISHED!`);
    ...
    console.log(`\n✨ ALL SKINS NOW HAVE COMPLETE MARKET DATA + HISTORY!`);

  } catch (error) {
    console.error('❌ Error in complete skin data fix with history:', error);
  } finally {
    await prisma.$disconnect();
  }
}

fixAllSkinsCompleteWithHistory();
