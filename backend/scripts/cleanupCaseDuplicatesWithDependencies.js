// /backend/scripts/cleanupCaseDuplicatesWithDependencies.js — [Backend]
// {/* Clean up case duplicates by first removing dependent data */}
import "dotenv/config";
import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

async function cleanupCaseDuplicatesWithDependencies() {
  console.log("🧹 Cleaning up case duplicates with dependencies...");
  ...
  try {
    ...
    for (const [normalizedName, group] of duplicates) {
      ...
      for (const removeCase of removeCases) {
        ...
        console.log(`    🗑️  Deleting dependent data for case ID ${removeCase.id}...`);
        
        // Delete CaseSupply
        const supplyCount = await prisma.caseSupply.deleteMany({
          where: { caseId: removeCase.id }
        });
        console.log(`      - Deleted ${supplyCount.count} CaseSupply records`);
        
        // Delete CasePriceHistory
        const priceHistoryCount = await prisma.casePriceHistory.deleteMany({
          where: { caseId: removeCase.id }
        });
        console.log(`      - Deleted ${priceHistoryCount.count} CasePriceHistory records`);
        
        // Delete CaseSkin
        const caseSkinCount = await prisma.caseSkin.deleteMany({
          where: { caseId: removeCase.id }
        });
        console.log(`      - Deleted ${caseSkinCount.count} CaseSkin records`);
        
        // Delete CasePortfolio
        const casePortfolioCount = await prisma.casePortfolio.deleteMany({
          where: { caseId: removeCase.id }
        });
        console.log(`      - Deleted ${casePortfolioCount.count} CasePortfolio records`);
        
        // Now safely delete the case
...
cleanupCaseDuplicatesWithDependencies();
