...
async function fixAllSkinsWithHistory() {
  try {
    console.log('🚀 Starting COMPLETE skin data fix with PRICE & QUANTITY HISTORY...');
    
    // Get skins that have market data but no history
    const skins = await prisma.skin.findMany({
      ...
      take: 500 // Process 500 skins at a time
    });

    console.log(`📊 Processing ${skins.length} skins for history creation...`);
    ...
    for (const skin of skins) {
      try {
        ...
        const priceHistorySuccess = await createPriceHistory(skin.id, skin.priceLatest);
        if (priceHistorySuccess) priceHistoryCount++;

        // Create quantity history
        const quantityHistorySuccess = await createQuantityHistory(skin.id, skin.offerVolume);
        if (quantityHistorySuccess) quantityHistoryCount++;

        processedCount++;
        
        if (processedCount % 50 === 0) {
          console.log(`✅ Processed ${processedCount}/${skins.length} skins...`);
        }
    ...
    console.log(`\n🎉 HISTORY CREATION COMPLETED!`);
    console.log(`📊 Final Statistics:`);
    console.log(`   • Skins processed: ${processedCount}`);
    console.log(`   • Price history created: ${priceHistoryCount}`);
    console.log(`   • Quantity history created: ${quantityHistoryCount}`);
    console.log(`\n✨ ALL SKINS NOW HAVE COMPLETE HISTORY DATA!`);

  } catch (error) {
    console.error('❌ Error in history creation:', error);
  } finally {
    await prisma.$disconnect();
  }
}

fixAllSkinsWithHistory();
