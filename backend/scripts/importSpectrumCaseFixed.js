...
async function run() {
  try {
    // Find EXACT case name
    const c = await prisma.case.findUnique({ where: { name: "Spectrum Case" } });
    
    if (!c) {
      console.log("❌ Spectrum Case not found");
      const allCases = await prisma.case.findMany({ where: { name: { contains: "Spectrum" } } });
      console.log("Available Spectrum cases:", allCases.map(x => x.name));
      return;
    }

    console.log(`📦 Found: ${c.name} (ID: ${c.id})`);
