...
async function run() {
  try {
    const c = await prisma.case.findUnique({ where: { name: "Gamma Case" } });
    
    if (!c) {
      console.log("❌ Gamma Case not found");
      return;
    }
    ...
    console.log(`✅ Gamma Case: ${created} created, ${added} linked`);
  } catch (e) {
    console.error("❌", e);
  } finally {
