// backend/scripts/fetchRealHistoricalData.js — [Backend]
// {/* Script to fetch REAL historical data from SteamWebAPI.com */}
...
async function fetchRealHistoricalData() {
  console.log("📊 [REAL-HISTORY] Starting real historical data fetch...");
  console.log(`📅 [REAL-HISTORY] Start time: ${new Date().toISOString()}`);
  
  if (!STEAM_WEB_API_KEY) {
    console.error("❌ [REAL-HISTORY] STEAM_API_KEY not found, cannot proceed");
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`📦 [REAL-HISTORY] Found ${skins.length} skins to process`);

    for (const skin of skins) {
      try {
        console.log(`\n🔄 [REAL-HISTORY] Processing skin ${skin.id}: ${skin.marketHashName}`);
        ...
        if (totalProcessed % 25 === 0) {
          console.log(`\n📊 [REAL-HISTORY] Progress: ${totalProcessed}/${skins.length} skins processed`);
    ...
    console.log("\n============================================================");
    console.log("✅ [REAL-HISTORY] Real historical data fetch completed!");
    ...
    console.error("❌ [REAL-HISTORY] Fatal error during fetch:", error);
    process.exit(1);
  } finally {
