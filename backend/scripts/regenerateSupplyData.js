// /backend/scripts/regenerateSupplyData.js — [Backend]
// {/* Regenerate supply data with correct logic */}
import "dotenv/config";
import { PrismaClient } from "@prisma/client";

const prisma = new PrismaClient();

async function regenerateSupplyData() {
  console.log("🔧 Regenerating supply data with correct logic...");
  ...
  try {
    ...
    for (const caseItem of cases) {
      ...
      // Generate realistic supply history
      ...
      if (supplyHistory.length > 0) {
        ...
        console.log(`  ✅ Generated ${supplyHistory.length} supply data points`);
        console.log(`  📊 Final: Drops=${totalDropped}, Unboxed=${totalUnboxed}, Remaining=${remainingSupply}`);
      }
    }
    
    console.log(`\n🎉 Regenerated supply data for all cases!`);
    
  } catch (error) {
    console.error("❌ Error regenerating supply data:", error);
  } finally {
    await prisma.$disconnect();
  }
}

regenerateSupplyData();
